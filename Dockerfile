...
[ -s "$NVM_DIR/nvm.sh" ] && \. "$NVM_DIR/nvm.sh" && nvm install 8.4.0 && nvm use 8.4.0
RUN git clone https://github.com/netbeen/wealth-manager.git && \
cd wealth-manager && \
~/.nvm/versions/node/v8.4.0/bin/npm install && ~/.nvm/versions/node/v8.4.0/bin/npm install -g egg-scripts && \
